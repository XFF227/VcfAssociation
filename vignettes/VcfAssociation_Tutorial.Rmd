...
If you already have a phenotype file, you can read and merge it directly
with `read_phenotypes()`. If not, `generate_phenotype()` can create a
simple phenotype from a specific variant in the VCF.
...
We then write this phenotype to a temporary CSV file and use
`read_phenotypes()` to merge it with the genotype table:
